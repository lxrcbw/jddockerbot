...
async def logbtn(conv, SENDER, path, msg):
    ...
    try:
        ...
        if res == 'cancle':
            ...
            msg = await client.edit_message(msg, '日志发送中，请注意查收')
        ...
        if event.message.file:
            ...
            async with client.conversation(SENDER, timeout=30) as conv:
                ...
                if res == 'cancel':
                    ...
                    cmdtext = 'js {}/{} now'.format(_ScriptsDir, filename)
                    subprocess.Popen(
...
async def mystart(event):
    ...
    msg = '''使用方法如下：
    /start 开始使用本程序
    /node 执行js脚本文件，执行js，需输入绝对路径。即可进行执行。该命令会等待脚本执行完，期间不能使用机器人，建议使用snode命令。
